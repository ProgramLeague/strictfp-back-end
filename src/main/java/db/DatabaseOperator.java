...
public class DatabaseOperator {
	public Article getArticle(int Id) throws SQLException {
		ResultSet resultSet = Adapter.getAdapter().select("article", "*", new Pair("Id", "=" + Id));
		Article article;
		...
		ResultSet writerResultSet =
				Adapter.getAdapter().select("writer", "*", new Pair("Id", "=" + resultSet.getInt("writerId")));
		...
		for (String thisTag : tags)
			tags1.add(new Tag(thisTag));
		article = new Article(Id, resultSet.getInt("pdate"), writer, tags1, resultSet.getString("title"),
